...
export class ShipperComponent {
  ...
  onShowTable() {
    ...
    this.http.post('https://localhost:44377/' + 'api/Shippers/DanhSachShipperTheoThangNam_Linq', this.dateReq).subscribe(result => {
      var res: any = result;
      console.log(result)
      this.list = res.data
    }, error => {
  ...
  convert(lst) {
    ...
    lst.forEach(element => {
      ...
      item.push(parseFloat(parseFloat(element.tongSoTien.toString()).toFixed(2)));
      item.push("pink");
      res.push(item);
    });
    return res;
